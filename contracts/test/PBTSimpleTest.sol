...
contract PBTSimpleTest is Test {
  event PBTMint(uint256 indexed chipAddress, address indexed tokenId);
  ...
  function _createSignature(bytes memory payload) private returns (bytes memory signature) {
  ...
  function testMintTokenWithChip() public {
    ...
    bytes memory chipSignature1 = _createSignature(payload);

    pbt.addChipToWhitelist(chipSignature1, blockNumber);
