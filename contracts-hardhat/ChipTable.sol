...
interface IChipTable is IERC165 {
   ...
   */
     ...
     * Permissions: Owner
     */
    function registerChipIds(address tsmAddress, bytes32[] calldata chipIds)
        external;
...
contract ChipTable is IChipTable, Context, Ownable {
    ...
    /*=== OWNER ===*/
    function registerTSM(address tsmAddress, string calldata uri)
        external
        override
        onlyOwner
    {
    ...
    function safeRegisterChipIds(
        ...
        require(_tsmExists(tsmAddress), "Owner: TSM does not exist");
        require(
            chipIds.length == signatures.length,
            "Owner: chipIds and signatures length mismatch"
        );
        for (uint256 i = 0; i < chipIds.length; i++) {
    ...
    function _registerTSM(address tsmAddress, string calldata uri) internal {
    ...
    function totalTSMs() external view override returns (uint256) {
        return _tsmCount;
    }
    ...
    function addChipIds(
        ...
        require(
            chipIds.length == signatures.length,
            "TSM: chipIds and signatures length mismatch"
        );
        for (uint256 i = 0; i < chipIds.length; i++) {
            _addChipSafe(tsmAddress, chipIds[i], signatures[i]);
        }
    ...
    function _isValidChipSignature(
        address tsmAddress,
        bytes32 chipId,
        bytes calldata signature
    ...
    function _addChipSafe(
        address tsmAddress,
        bytes32 chipId,
        bytes calldata signature
    ) internal {
        require(
            _isValidChipSignature(tsmAddress, chipId, signature),
            "Chip: chip signature invalid"
        );
    ...
    function chipTSM(bytes32 chipId) public view override returns (address) {
    ...
    function chipExists(bytes32 chipId) public view override returns (bool) {
